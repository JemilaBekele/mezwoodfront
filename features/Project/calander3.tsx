...
const router = useRouter();
  ...
  const handleDragEnd = useCallback((event: DragEndEvent) => {
    ...
    // ALL stages use the project selection dialog
    if (stageData && stageData.row) {
      setDragPending({
        sourceDate,
        targetDate,
        stageData,
  ...
  const handleRebuild = async () => {
    if (!window.confirm("Rebuild the FULL capacity calendar from current projects?")) return;
    try {
      setResetting(true);
...
// Get all active projects with remaining quantity for the SPECIFIC stage being dragged
const getAvailableProjectsForStage = useCallback((stageType: CapacityStage) => {
    ...
    if (r.stage === stageType) {
      ...
      allocs.forEach((a: any) => {
        ...
        if (p && p.status !== "COMPLETED") {
          // Get the allocated units for THIS stage from this allocation
          ...
          if (allocatedUnits > 0 && !projectMap.has(p.id)) {
            projectMap.set(p.id, {
              ...
              quantity: allocatedUnits, // Show the actual allocated units for this stage
            });
          }
        }
      });
          ...
          <div className="cc-mast__meta">
            ...
            <div className="cc-actions">
              ...
              >
                ...
                {resetting ? "Rebuilding…" : "Full Rebuild"}
              </Button>
            </div>
          </div>
        </div>
  ...
  if (active) {
    router.push(`/dashboard/capacityday/${cell.key}`);
                    ...
                    <div className="cc-cell__top">
                      ...
                      {active && (
                        <span className="cc-cell__peak" style={{ background: heat }}>
                          ...
                          const stageBar = (
                            <div
                              ...
                              style={{ 
                                ...
                                borderColor: isDesignOverCapacity ? "var(--cc-over)" : undefined,
                                backgroundColor: isDesignOverCapacity ? "color-mix(in srgb, var(--cc-over) 15%, transparent)" : undefined,
                              } as React.CSSProperties}
                            ...
                            >
                              <span
                                ...
                                style={{ 
                                  ...
                                  opacity: isDesignOverCapacity ? 0.5 : 0.34,
                                }}
                              />
                              ...
                              <div style={{ display: 'flex', alignItems: 'center', gap: '4px', marginLeft: 'auto' }}>
                                ...
                                {isDesignOverCapacity && (
                                  <span className="cc-design-over-badge" style={{ color: 'var(--cc-over)' }}>
                                    +{Math.round((totalFrac - 1) * 100)}%
                                  </span>
                                )}
                                <span className="cc-cell__rowpct">{clampPct(totalFrac * 100)}</span>
                              </div>
                            </div>
                          );
      ...
      <StageRescheduleDialog
        open={!!dragPending}
        projects={dragPending ? getAvailableProjectsForStage(dragPending.stage) : []}
