...
// Helper function to get the Metal Works stage from a project
...
export const projectColumns: ColumnDef<IProject>[] = [
    ...
    cell: ({ cell, row }) => {
      ...
      return (
        <div
          ...
          onClick={() =>
            router.push(
              `/dashboard/Stage/metalworks/view?id=${row.original?.id}`
            )
          }
        >
  ...
  },
  {
    ...
    header: ({ column }) => (
      <DataTableColumnHeader column={column} title='Metal Works Progress' />
    ),
    cell: ({ row }) => {
      ...
      if (!stage) {
        return <span className="text-muted-foreground text-sm">No Metal Works stage</span>;
      }
